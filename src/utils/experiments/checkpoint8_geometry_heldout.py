...
# ==========================================================
# LOAD THE FROZEN EXPERIMENT — no changes allowed
# ==========================================================
...
DEV_RANKING = pd.DataFrame(FROZEN['full_ranking'])

# ==========================================================
# FIXED SETTINGS — identical everywhere
# ==========================================================
FS = 16000
STFT_SIZE = 512
STFT_SHIFT = 128
...
# NEW geometries, unseen during development (which used G-A: 5x4x2.8)
GEOMETRIES = {
    'G-C': [6.0, 5.0, 3.0],
    'G-D': [10.0, 6.0, 3.5],
}

# SAME broad ranges as development — geometry is the only changed factor
...
RESULTS_CSV = 'results/tables/checkpoint8_geometry_heldout_results.csv'
os.makedirs('results/tables', exist_ok=True)

# NEW seed range — must not overlap Checkpoint 6 (201-212) or 7 (301-312)
SEED_BASE = 400

CSV_COLUMNS = [
    'room_id', 'geometry', 'rt60_target', 'rt60_achieved', 'distance_m', 'replicate', 'seed',
    'snr_condition', 'utterance', 'config_id', 'delay', 'taps',
    'stoi', 'si_sdr_db', 'runtime_s', 'numerical_failure', 'signal_failure', 'warnings'
]

# ==========================================================
# Shared helpers — identical logic to Checkpoints 4-7
# ==========================================================
...
# ==========================================================
# Same held-out test utterances as Checkpoint 7 (disjoint from development)
# ==========================================================
...
print(f"Using {len(UTTERANCES)} TEST utterances: {UTTERANCES}")
print(f"Geometries: {GEOMETRIES}")

total_estimate = len(GEOMETRIES) * len(RT60_TARGETS) * len(DISTANCES) * N_REPLICATES * len(SNR_CONDITIONS) * len(UTTERANCES) * (1 + len(CONFIGS))
print(f"Estimated total rows: {total_estimate}")

# ==========================================================
# MAIN LOOP — two geometries x full RT60/distance/replicate/SNR grid
# ==========================================================
room_metadata_log = {}
combo_index = 0

for geom_name, room_dim in GEOMETRIES.items():
    for rt60_target in RT60_TARGETS:
        for distance in DISTANCES:
            for replicate in range(N_REPLICATES):
                combo_index += 1
                seed = SEED_BASE + combo_index
                room_id = f"H_{geom_name}_rt{rt60_target}_d{distance}_rep{replicate}"

                rir_array, room_meta = generate_room_rir(room_dim, FS, rt60_target, distance, MIC_SPACING, seed)
                room_meta['seed'] = seed
                room_meta['geometry_name'] = geom_name
                ...
                for snr_db in SNR_CONDITIONS:
                    ...
                    for utt_path in UTTERANCES:
                        ...
                        if key not in done_keys:
                            ...
                            append_row({
                                'room_id': room_id, 'geometry': geom_name, 'rt60_target': rt60_target,
                    ...
                    print(f"  [{room_id} | {snr_label}]: done")

with open('results/tables/checkpoint8_room_metadata.json', 'w') as f:
    json.dump(room_metadata_log, f, indent=2)

print(f"\nCheckpoint 8 data collection complete. {len(done_keys)} total rows in {RESULTS_CSV}")

# ==========================================================
# ANALYSIS — same structure as Checkpoint 7
# ==========================================================
...
cell_avg.to_csv('results/tables/checkpoint8_cell_level.csv', index=False)

geom_macro = cell_avg.groupby('config_id')['cell_stoi_improvement'].mean().reset_index()
geom_macro.rename(columns={'cell_stoi_improvement': 'macro_stoi_improvement'}, inplace=True)
geom_macro = geom_macro.sort_values('macro_stoi_improvement', ascending=False).reset_index(drop=True)
geom_macro['rank'] = geom_macro.index + 1
geom_macro.to_csv('results/tables/checkpoint8_ranking.csv', index=False)

print("\n=== Geometry-held-out ranking (macro STOI improvement) ===")
print(geom_macro.to_string(index=False))

merged_ranks = DEV_RANKING[['config_id', 'rank']].merge(
    geom_macro[['config_id', 'rank']], on='config_id', suffixes=('_dev', '_geom')
)
tau, p_value = kendalltau(merged_ranks['rank_dev'], merged_ranks['rank_geom'])
...
for i in range(len(configs_list)):
    for j in range(i + 1, len(configs_list)):
        ...
        g1 = merged_ranks.loc[merged_ranks.config_id == c1, 'rank_geom'].iloc[0]
        g2 = merged_ranks.loc[merged_ranks.config_id == c2, 'rank_geom'].iloc[0]
        pairs_total += 1
        if (dev1 < dev2) == (g1 < g2):
            pairs_agree += 1
pairwise_agreement = pairs_agree / pairs_total

top3_geom = geom_macro.head(3)['config_id'].tolist()
top3_overlap = len(set(TOP3_DEV) & set(top3_geom))

best_of_9 = geom_macro['macro_stoi_improvement'].max()
best_of_top3_dev = geom_macro[geom_macro['config_id'].isin(TOP3_DEV)]['macro_stoi_improvement'].max()
selection_regret = best_of_9 - best_of_top3_dev

# Runtime summary
runtime_by_config = configs_df.groupby('config_id')['runtime_s'].agg(['mean', 'std', 'count']).round(3).reset_index()
...
transfer_summary = {
    ...
    'geometry_heldout_top3': top3_geom,
    ...
    'best_of_nine_geom_heldout': float(best_of_9),
    'best_of_dev_top3_on_geom_heldout': float(best_of_top3_dev),
    'selection_regret_stoi': float(selection_regret),
    'geometry_heldout_ranking': geom_macro.to_dict(orient='records'),
    'numerical_failures': int(df['numerical_failure'].sum()),
    'signal_failures': int(df['signal_failure'].fillna(False).sum()),
    'total_runs': len(df),
    'runtime_mean_by_config': runtime_by_config.set_index('config_id')['mean'].to_dict(),
    'failure_by_geometry': failures['geometry'].value_counts().to_dict() if len(failures) > 0 else {},
    'failure_by_rt60': failures['rt60_target'].value_counts().to_dict() if len(failures) > 0 else {},
    'failure_by_distance': failures['distance_m'].value_counts().to_dict() if len(failures) > 0 else {},
    'note': 'Tests unseen geometries within the same Pyroomacoustics simulator; does not compare simulation paradigms.',
}

with open('results/tables/checkpoint8_transfer_summary.json', 'w') as f:
    json.dump(transfer_summary, f, indent=2)

print("\n=== TRANSFER SUMMARY (Geometry-Held-Out) ===")
print(f"Development top-3: {TOP3_DEV}")
print(f"Geometry-held-out top-3: {top3_geom}")
